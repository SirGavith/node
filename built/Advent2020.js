...
class Advent2020 {
    ...
    static Day2(d) {
        console.log(d.SplitLines().reduce((count, password) => {
            ...
            const range = p[0].split('-').toIntArray(), letter = p[1][0], pass = p[2];
            console.log(range, letter, pass);
            //(pass.split(letter).length - 1).InRangeEq(range[0], range[1]).IsTrue(() => count++)
            if ((pass[range[0] - 1] == letter) != (pass[range[1] - 1] == letter))
                count++;
            return count;
        }, 0));
    }
    ...
    static Day4(d) {
        ...
        data.reduce((count, passport) => {
            ...
            return passport['byr']?.toInt().InRangeEq(1920, 2002) &&
                ...
                passport['hgt'] && ((hgt.endsWith('cm') && hgt.slice(0, -2).toInt().InRangeEq(150, 193)) ||
    ...
    static Day5(data) {
        const ids = data.SplitLines().map(d => d.slice(0, -3).ReplaceMap({ 'F': '0', 'B': '1' }).toInt(2) * 8 +
            d.slice(-3).ReplaceMap({ 'L': '0', 'R': '1' }).toInt(2)).sort((a, b) => a - b);
